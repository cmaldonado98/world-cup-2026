...
--  Nuevo:   "[CÓDIGO][N]"   →  "ARG1",   "FWC",   "FWC1"
--
--  Cambios estructurales:
--    · Sección FIFA  → FWC  (FIFA0 → FWC / logo; FIFA1..19 → FWC1..19)
...
--  "FIFA0"  → "FWC"    (cromo Logo)
--  "FIFA1"  → "FWC1"
--  "FIFA19" → "FWC19"
UPDATE public.user_cards
  SET card_id = CASE
    WHEN card_id = 'FIFA0' THEN 'FWC'
...
--  Acepta: "FWC" (logo), "FWC1"–"FWC19", "MEX1"–"MEX20", "CC1"–"CC14"
ALTER TABLE public.user_cards
  ADD CONSTRAINT user_cards_card_id_check
  CHECK (card_id ~ '^[A-Z]{2,5}[0-9]{0,3}$');
